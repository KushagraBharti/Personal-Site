...
export const portfolioProjects: PortfolioProject[] = [
  ...
  },
  {
    slug: "personal-site",
    order: 11,
    title: "Personal Site + Tracker",
    summary:
      "A full-stack personal platform with a statically prerendered AI-readable portfolio, a private Supabase task planner, Google Calendar sync, Poke MCP tools, and Realtime-backed tracker refresh.",
    description: [
      "Product shape: Personal Site is two products in one repo: a public portfolio for humans, recruiters, crawlers, and AI agents, plus a private authenticated tracker for real task and calendar workflows.",
      "Route model: `/` serves the public portfolio, `/ai` serves the AI-readable profile, `/tracker` serves the private app, `/api` serves public portfolio APIs, `/api/private` serves tracker APIs, and `/api/mcp` exposes the Poke-compatible tracker MCP endpoint.",
      "Portfolio content model: Backend TypeScript content is the source of truth for profile copy, about text, education, experience, projects, writings, film/media, AI prompts, and public source links, with shared frontend/backend contracts around the exported snapshot.",
      "Static export surface: The sync pipeline emits `index.html`, `ai.html`, `llms.txt`, `portfolio.json`, `version.json`, `robots.txt`, `sitemap.xml`, `introBootstrap.ts`, and `portfolioSnapshotBootstrap.ts` from the same canonical backend snapshot.",
      "Public frontend runtime: The homepage ships as a statically prerendered React shell, hydrates through Vite, lazy-loads `/ai` and `/tracker`, keeps fonts self-hosted, serves optimized AVIF/WebP image variants through portfolio image helpers, and defers media-heavy homepage enhancements plus the 3D hero model until after first paint.",
      "Live portfolio widgets: GitHub stats and weather stay behind Express APIs, not browser-only calls. GitHub stats prefer the GraphQL contribution path with cache and fallback strategies; weather uses OpenWeather plus backend geo/query fallback and never triggers a browser location prompt.",
      "Tracker auth boundary: `/tracker` uses Supabase email/password auth in the browser, but tracker data flows through backend-owned private APIs. The frontend resolves fresh Supabase access tokens before private requests and before joining Realtime topics.",
      "Tracker UI surface: The tasks hub has sidebar lists, list counts, all-tasks and per-list views, open/completed sections, subtasks, notes/details, due dates, recurrence controls, quick date actions, local hide-completed preferences, per-list sort modes, and drag reorder when custom sort is active.",
      "Task and list API: Express private routes own tracker bootstrap, list create/update/reorder/delete, task create/update/reorder/delete, completion toggles, sort preferences, calendar status/actions, and every ownership check through `requireUser` plus the Supabase admin client.",
      "Task model: Tasks carry list links, optional parent-task links, details, `due_at`, `due_timezone`, completion state, sibling `sort_order`, and recurrence fields. The backend validates list ownership, same-list parent relationships, full sibling reorder payloads, recursive delete cleanup, and queue side effects.",
      "Date and recurrence semantics: Date-only tasks normalize to 10 PM in the selected timezone, offsetless datetimes stay local to the task timezone, absolute ISO datetimes keep their instant, recurring tasks require due dates, completion can create the next occurrence, and cron reconciliation repairs completed recurring tasks that are missing their next open copy.",
      "Supabase data model: The live schema includes user-scoped task lists, tasks, task sort preferences, list sync settings, Google calendar connection public rows, encrypted token secret rows, sync jobs, sync runs, Google event link tables, recurring projection link tables, RPC job helpers, and Realtime Broadcast objects.",
      "Google Calendar connection: OAuth uses signed expiring state, encrypted token storage, Google account metadata, dedicated Tracker Tasks calendar creation/repair, selected-calendar status, watch renewal, webhook validation, and disconnect cleanup.",
      "Google event model: Tracker-owned Google events use deterministic IDs, private extended metadata, timed events, all-day date-only events, `[Done]` titles for completed non-recurring tasks, `[Upcoming]` projected recurring events, primary event links, and projection link rows.",
      "Calendar sync engine: Calendar work runs through backend-owned jobs with lanes, run IDs, priorities, dedupe keys, claim/complete/fail RPCs, stale-running-job recovery, bounded drains for serverless limits, and live/manual/rebuild/list-backfill/inbound-delta/watch-renew entrypoints.",
      "Reconciliation behavior: App-side reconciliation scans due synced-list tasks, Google-side reconciliation reads tracker-owned calendar events, rebuild mode clears and reconstructs the Tracker Tasks calendar, and inbound deltas restore cancelled/deleted tracker-owned Google events instead of importing arbitrary calendar edits as tasks.",
      "Realtime refresh: Supabase database triggers emit small `tracker_change` Broadcast invalidation events on private `tracker:user:<auth_user_id>` topics for task/list/sort/list-sync/calendar-public changes. The frontend debounces those signals and refetches canonical backend bootstrap/calendar state.",
      "MCP/Poke surface: `/api/mcp` has bearer auth, origin checks, optional Poke-user allowlisting, owner scoping, GET/SSE compatibility, and tools for tracker snapshots, active tasks, completed tasks, create/update/complete/uncomplete/delete, and manual calendar sync.",
      "MCP visibility model: Assistant tools only see non-archived task lists with Google Calendar sync enabled, require exact list IDs or normalized list names for writes, keep task payloads compact, preserve recurrence/date semantics, and require explicit confirmation for destructive subtree deletes.",
      "Security boundary: Public portfolio exports never include private tracker data. Frontend env stays limited to public `VITE_*` Supabase/API values, server-only Supabase/Google/MCP secrets remain backend-side, production CORS stays constrained to known frontend aliases unless preview origins are explicitly enabled, and Supabase RLS/Realtime policies guard private topics.",
      "Deployment and operations: Frontend and backend run as separate Vercel projects, backend cron routes handle calendar sync, watch renewal, and recurring-task repair, Bun is the normal app runtime/tooling path, and repo-level verification uses npm where the Windows/OneDrive test toolchain is more reliable.",
      "Verification surface: Backend tests cover portfolio routes, live widgets, auth middleware, tracker list/task routes, task services, Google Calendar services, cron routes, MCP routes, and MCP service behavior. Frontend tests cover route loading, portfolio rendering/API caching, tracker auth shell, tasks-hub APIs, hooks, components, and date-time helpers.",
      "Engineering thesis: The hard part is keeping static public content, private server-owned tracker state, Supabase Auth/RLS/Realtime, Google Calendar side effects, serverless queue processing, and assistant-facing MCP tools coherent without leaking data or trusting stale browser state.",
    ],
    tags: [
      "TypeScript",
      "Node.js",
