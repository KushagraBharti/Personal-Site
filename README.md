...
The public site lives at `/`. It prerenders crawlable homepage HTML at build time, then the browser renders an eager hero shell first and loads the 3D hero model plus below-the-fold sections in the background without skeleton placeholders.
...
Portfolio content starts in `backend/src/portfolio/content`. The frontend should render that data instead of becoming a second content source.
