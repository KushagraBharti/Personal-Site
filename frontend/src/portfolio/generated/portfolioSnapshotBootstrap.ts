...
export const portfolioSnapshotBootstrap: PortfolioSnapshot = {
  ...
  "projects": [
    ...
    },
    {
      ...
      "description": [
        "Context: Built F1RL as a custom racing AI lab around Monza, combining reinforcement learning, physics simulation, controller search, CUDA acceleration, telemetry, replay, and real Formula 1 timing references instead of training on an off-the-shelf toy environment.",
        "Problem: A racing agent has to solve long-horizon control: accelerate aggressively, brake late but stably, rotate through chicanes, manage curbs, recover from imperfect lines, avoid wall/off-track failures, and complete a full lap quickly. The challenge was not just producing motion; it was producing fast, legal, repeatable racing behavior.",
        "System architecture: Built a Gymnasium-compatible Monza environment around a shared simulator, with normalized observations, action modes, checkpoint-valid lap logic, Stable-Baselines3 PPO hooks, benchmark tooling, telemetry, replay, and evolutionary controller search all using the same simulation core.",
        "Physics architecture: Built the simulator around richer F1-style vehicle dynamics, including tire slip-angle behavior, tire-force saturation, weight transfer, automatic gear/torque curves, brake-bias instability, curb/surface grip modifiers, and oriented car-body collision checks.",
        "Tire and weight-transfer model: Added physics that makes corner entry, trail braking, understeer/oversteer, brake release, throttle timing, and high-speed turning materially affect lap behavior instead of reducing driving to simple steering and flat acceleration.",
        "Track and calibration: Modeled Monza with track geometry, checkpoint progression, collision/off-track logic, and real F1 telemetry-calibrated physics so lap times, speed profiles, braking behavior, and turning dynamics are grounded against a real reference instead of an arbitrary reward score.",
        "Observation design: Engineered ray-cast and racing-line state features so controllers can reason from speed, heading error, lateral error, lap progress, ray distances, lookahead heading, target speed, brake demand, target-speed drops, curvature, and section-aware braking signals.",
        "PPO infrastructure: Built Stable-Baselines3 PPO training/evaluation with PyTorch/CUDA support, checkpointing, vectorized environments, curriculum/state-library starts, metadata-aware evaluation, reward overrides, and benchmark tooling.",
        "Learning strategy: PPO infrastructure was useful but did not solve the full-lap task by itself, so the system became a hybrid RL/search platform: PPO provided the RL environment/training foundation, while evolutionary search became the main discovery engine for fast driving behavior.",
        "GPU evolutionary search: Built a CUDA-scale evolutionary controller-search workflow that evaluates large populations of driving controllers, scores full-lap behavior, preserves elite candidates, applies mutation/crossover, and pushes the population toward faster valid laps.",
        "Search optimization: Tuned reward functions, scoring profiles, survival floors, mutation/crossover behavior, elite selection, and candidate promotion as the system moved from basic lap completion to high-speed lap optimization.",
        "Scale and result: Ran a 2000-population, 150-generation, 25k-step GPU-fused search over 300,000 controller candidates, producing a fastest evolved Monza lap of 81.233s.",
        "Population quality: The 2000x150 run ended with 451 valid laps in the final generation, a 22.55% final-generation valid-lap rate, a 103.08s final-generation average valid lap, 216.53kph final-generation average pace, and 808.75 candidates/sec throughput.",
        "Verification and replay: Generated selected replay telemetry for the fastest candidate and exported a pygame-window-style GIF of the 81.233s lap, keeping the result visually inspectable instead of only reporting a scalar lap time.",
        "Process breakthrough: The project moved from a PPO plateau, to CPU evolutionary search around 89s, to GPU evolutionary search at 81.233s, with the main technical leap coming from better physics, larger candidate populations, and more aggressive search/scoring iteration.",
        "Impact and outcome: Converted the project into a complete racing AI system: a custom RL environment, realistic-enough vehicle physics, PPO infrastructure, GPU-scale evolutionary search, real F1 calibration, replayable results, and a clear path toward learned-policy distillation.",
        "Next RL direction: The strongest next step is using verified GPU ES trajectories as a transition dataset for behavior cloning, SAC, or TD3, then injecting learned actors back into the evolutionary loop to improve consistency and close the final gap to the target time.",
        "What made it hard: The hard part was not calling PPO or running a search loop; it was designing a simulator where physics mattered, exposing learnable state, tuning rewards and search objectives, scaling to hundreds of thousands of candidates, and turning replay analysis into algorithmic changes that produced an 81.233s lap."
      ],
      "tags": [
        "Physics Simulation",
        "Reinforcement Learning",
